        ...
        if (cs == 0x00 && ln == nextline)
        {
            ...
            while (possible_command.size() > 0)
            {
                ...
                if (!uploading || upload_stream != new_message.stream)
                {
                    ...
                    {
                        if (gcode->g == 53)
                        { // G53 makes next movement command use machine coordinates
                    ...
                    if (gcode->has_m)
                    {
                        ...
                        case 30: // end of program
                            if (!THEKERNEL->is_grbl_mode())
                                break; // Special case M30 as it is also delete sd card file so only do this if in grbl mode
                            // fall through to M2
                        case 2:
                        {
                        ...
                        case 504: // save to specific config override file
                        {
                            ...
                            if (arg.empty())
                                arg = "/sd/config-override";
                            else
                                arg = "/sd/config-override." + arg;
                        ...
                        case 503:
                        { // M503 display live settings and indicates if there is an override file
