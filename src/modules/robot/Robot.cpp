...
void Robot::load_config()
{
    ...
    for (size_t a = 0; a < MAX_ROBOT_ACTUATORS; a++)
    {
        ...
        { // step and dir must be defined, but enable is optional
            if (a <= Z_AXIS)
            {
...
void Robot::print_position(uint8_t subcode, std::string &res, bool ignore_extruders) const
{
    ...
    else
    {
        ...
        if (compensationTransform)
            compensationTransform(mpos, true); // get inverse compensation transform

        if (subcode == 1)
        { // M114.1 print realtime WCS
        ...
        { // M114.2 print realtime Machine coordinate system
            n = snprintf(buf, sizeof(buf), "MCS: X:%1.4f Y:%1.4f Z:%1.4f", mpos[X_AXIS], mpos[Y_AXIS], mpos[Z_AXIS]);
        }
        else if (subcode == 3)
        { // M114.3 print realtime actuator position
            // get real time current actuator position in mm
            ActuatorCoordinates current_position{
                actuators[X_AXIS]->get_current_position(),
                actuators[Y_AXIS]->get_current_position(),
    ...
    {
        n = 0;
        if (ignore_extruders && actuators[i]->is_extruder())
            continue; // don't show an extruder as that will be E
        if (subcode == 4)
        { // M114.4 print last milestone
            n = snprintf(buf, sizeof(buf), " %c:%1.4f", 'A' + i - A_AXIS, machine_position[i]);
        }
        else if (subcode == 2 || subcode == 3)
        { // M114.2/M114.3 print actuator position which is the same as machine position for ABC
...
void Robot::check_max_actuator_speeds()
{
    for (size_t i = 0; i < n_motors; i++)
    {
        if (actuators[i]->is_extruder())
            continue; //extruders are not included in this check
...
void Robot::on_gcode_received(void *argument)
{
    ...
    if (gcode->has_g)
    {
        ...
        case 59:
            ...
            if (gcode->g == 59 && gcode->subcode > 0)
            {
                ...
                if (current_wcs >= MAX_WCS)
                    current_wcs = MAX_WCS - 1;
            }
            break;
    ...
    else if (gcode->has_m)
    {
        ...
        case 92: // M92 - set steps per mm
            ...
            {
                if (actuators[i]->is_extruder())
                    continue; //extruders handle this themselves
        ...
        { // M665 set optional arm solution variables based on arm solution.
            ...
            else if (gcode->has_letter('U'))
            { // or set mm_per_line_segment, not saved by M500
...
void Robot::process_move(Gcode *gcode, enum MOTION_MODE_T motion_mode)
{
    ...
    if (!next_command_is_MCS)
    {
        ...
        else
        {
            ...
            {
                if (!isnan(param[i]))
                    target[i] = param[i] + machine_position[i];
            }
        }
    }
    else
    ...
    if (this->use_workpiece_offset)
    {
        ...
        float machine_position_y = ((this->machine_position[Y_AXIS] - center_y) * cos_a - (this->machine_position[Z_AXIS] - center_z) * sin_a) + center_y;
        //reset_axis_position(machine_position_y, Y_AXIS);
    ...
    if (this->use_workpiece_offset)
    {
        ...
        moved = this->compute_arc(gcode, offset, target, delta_a >= 0 ? CCW_ARC : CW_ARC, delta_e);
        //}
        //else
        //{
...
void Robot::calculate_workpiece_offset(const float target[])
{
    ...
    float x_compensation = abs(std::get<X_AXIS>(wcs_offsets[1]) - std::get<X_AXIS>(wcs_offsets[2]));
    float y_compensation = abs(std::get<Y_AXIS>(wcs_offsets[1]) - std::get<Y_AXIS>(wcs_offsets[2]));
...
bool Robot::append_milestone(const float target[], float rate_mm_s)
{
    ...
    for (int i = 0; i < n_motors; ++i)
    {
        transformed_target[i] += babysteps[i];
        babysteps[i] = 0;
    }
    ...
    for (size_t actuator = 0; actuator < n_motors; actuator++)
    {
        ...
        if (d < 0.00001F || !actuators[actuator]->is_selected())
            continue; // no realistic movement for this actuator
...
bool Robot::append_line(Gcode *gcode, const float target[], float rate_mm_s, float delta_e)
{
    ...
    */
    ...
    if (segments > 1)
    {
        ...
        {
            if (THEKERNEL->is_halted())
                return false; // don't queue any more segments
