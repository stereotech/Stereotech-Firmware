...
bool Endstops::load_old_config()
{
    ...
    { // X_AXIS to Z_AXIS
        ...
        for (int j = MIN_PIN; j <= MAX_PIN; ++j)
        {
            ...
            if ((hinfo.home_direction && j == MIN_PIN) || (!hinfo.home_direction && j == MAX_PIN))
                hinfo.pin_info = info;
...
bool Endstops::load_config()
{
    ...
    for (auto cs : modules)
    {
        if (!THEKERNEL->config->value(endstop_checksum, cs, enable_checksum)->as_bool())
            continue;
...
{
    if (!is_delta && (!axis[X_AXIS] || !axis[Y_AXIS]))
...
uint32_t Endstops::read_endstops(uint32_t dummy)
{
    ...
    { // check all axis homing endstops
        if (e.pin_info == nullptr)
            continue; // ignore if not a homing endstop
        int m = e.axis_index;
...
void Endstops::home_xy()
{
    if (axis_to_home[X_AXIS] && axis_to_home[Y_AXIS])
    {
        ...
        if (homing_axis[X_AXIS].home_direction)
            delta[X_AXIS] = -delta[X_AXIS];
        if (homing_axis[Y_AXIS].home_direction)
...
void Endstops::home(axis_bitmap_t a)
{
    ...
    if (axis_to_home[Z_AXIS])
    {
        ...
        if (homing_axis[Z_AXIS].home_direction)
    ...
    if (homing_axis.size() > 3)
    {
        for (size_t i = A_AXIS; i < homing_axis.size(); ++i)
        {
            if (axis_to_home[i])
            {
                ...
                if (homing_axis[i].home_direction)
                    delta[i] = -delta[i];
...
void Endstops::set_homing_offset(Gcode *gcode)
{
    ...
    {
        if (!homing_axis[A_AXIS].homed)
        {
            gcode->stream->printf("error: Axis A must be homed before setting Homing offset\n");
            return;
        }
        homing_axis[A_AXIS].home_offset += (THEROBOT->to_millimeters(gcode->get_value('A')) - pos[A_AXIS]);
        homing_axis[A_AXIS].homed = false; // force it to be homed
    }

    gcode->stream->printf("Homing Offset: X %5.3f Y %5.3f Z %5.3f A %5.3f will take effect next home\n", homing_axis[X_AXIS].home_offset, homing_axis[Y_AXIS].home_offset, homing_axis[Z_AXIS].home_offset, homing_axis[A_AXIS].home_offset);
}
...
void Endstops::on_gcode_received(void *argument)
{
    ...
    if (gcode->has_g && gcode->g == 28)
    {
        ...
        case 1: // G28.1 set pre defined park position
            ...
            if (gcode->has_letter('X'))
                saved_position[X_AXIS] = gcode->get_value('X');
            if (gcode->has_letter('Y'))
                saved_position[Y_AXIS] = gcode->get_value('Y');
            break;
        ...
        case 3: // G28.3 is a smoothie special it sets manual homing
            ...
            else
            {
                ...
                if (homing_axis.size() > A_AXIS && homing_axis[A_AXIS].pin_info != nullptr && gcode->has_letter('A'))
                {
                    THEROBOT->reset_axis_position(gcode->get_value('A'), A_AXIS);
                    homing_axis[A_AXIS].homed = true;
                }
                if (homing_axis.size() > B_AXIS && homing_axis[B_AXIS].pin_info != nullptr && gcode->has_letter('B'))
                {
                    THEROBOT->reset_axis_position(gcode->get_value('B'), B_AXIS);
                    homing_axis[B_AXIS].homed = true;
                }
                if (homing_axis.size() > C_AXIS && homing_axis[C_AXIS].pin_info != nullptr && gcode->has_letter('C'))
                {
                    THEROBOT->reset_axis_position(gcode->get_value('C'), C_AXIS);
                    homing_axis[C_AXIS].homed = true;
                }
            }
            break;

        case 4:
        { // G28.4 is a smoothie special it sets manual homing based on the actuator position (used for rotary delta)
            // do a manual homing based on given coordinates, no endstops required
            ActuatorCoordinates ac{NAN, NAN, NAN};
            if (gcode->has_letter('X'))
            {
                ac[0] = gcode->get_value('X');
                homing_axis[X_AXIS].homed = true;
            }
            if (gcode->has_letter('Y'))
            {
                ac[1] = gcode->get_value('Y');
                homing_axis[Y_AXIS].homed = true;
            }
            if (gcode->has_letter('Z'))
            {
                ac[2] = gcode->get_value('Z');
                homing_axis[Z_AXIS].homed = true;
            }
        ...
        case 5: // G28.5 is a smoothie special it clears the homed flag for the specified axis, or all if not specifed
            ...
            else
            {
                ...
                if (homing_axis.size() > A_AXIS && gcode->has_letter('A'))
                    homing_axis[A_AXIS].homed = false;
                if (homing_axis.size() > B_AXIS && gcode->has_letter('B'))
                    homing_axis[B_AXIS].homed = false;
                if (homing_axis.size() > C_AXIS && gcode->has_letter('C'))
                    homing_axis[C_AXIS].homed = false;
            }
            break;
    ...
    else if (gcode->has_m)
    {
        ...
        case 206: // M206 - set homing offset
            ...
            for (auto &p : homing_axis)
            {
                ...
                if (gcode->has_letter(p.axis))
                    p.home_offset = gcode->get_value(p.axis);
            }
        ...
        case 666:
            ...
            { // M666 - set trim for each axis in mm, NB negative mm trim is down
                if (gcode->has_letter('X'))
                    trim_mm[0] = gcode->get_value('X');
                if (gcode->has_letter('Y'))
                    trim_mm[1] = gcode->get_value('Y');
                if (gcode->has_letter('Z'))
                    trim_mm[2] = gcode->get_value('Z');
...
void Endstops::on_set_public_data(void *argument)
{
    ...
    else if (pdr->second_element_is(home_offset_checksum))
    {
        float *t = static_cast<float *>(pdr->get_data_ptr());
        if (!isnan(t[0]))
            homing_axis[0].home_offset = t[0];
        if (!isnan(t[1]))
            homing_axis[1].home_offset = t[1];
        if (!isnan(t[2]))
            homing_axis[2].home_offset = t[2];
    }
}
