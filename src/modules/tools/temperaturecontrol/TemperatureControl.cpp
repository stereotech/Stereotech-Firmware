...
void TemperatureControl::on_gcode_received(void *argument)
{
    ...
    if (gcode->has_m)
    {
        ...
        { // M500 saves some volatile settings to config override file, M503 just prints the settings
        ...
        else if ((gcode->m == this->set_m_code || gcode->m == this->set_and_wait_m_code) && gcode->has_letter('S'))
        {
            ...
            {
                this->active = gcode->get_value('T') == this->pool_index;
            }
...
void TemperatureControl::on_set_public_data(void *argument)
{
    ...
    if (!pdr->second_element_is(this->name_checksum))
        return;
...
void TemperatureControl::set_desired_temperature(float desired_temperature)
{
    ...
    else if (last_target_temperature <= 0.0F)
    {
        ...
        this->iTerm = this->o;
        if (this->iTerm > this->i_max)
            this->iTerm = this->i_max;
        else if (this->iTerm < 0.0)
            this->iTerm = 0.0;
    }
...
void TemperatureControl::on_second_tick(void *argument)
{
    ...
    if (this->runaway_heating_timeout == 0 && this->runaway_range == 0)
        return;
    ...
    { // If we are not trying to heat, state is NOT_HEATING
        this->runaway_state = NOT_HEATING;
    }
    else
    {
